...
bool QHeifHandler::canRead() const
{
    ...
    QHeifHandler::Format mimeFormat = canReadFrom(*device());
...
void QHeifHandler::loadContext()
{
    ...
    heif_error error = readContext(context.get(),
    ...
    mReadState.reset(new ReadState{std::move(fileData),
                                   std::move(context),
                                   std::move(idList),
                                   currentIndex});
}
...
bool QHeifHandler::read(QImage* destImage)
{
    ...
    int idIndex = mReadState->currentIndex;
    Q_ASSERT(idIndex >= 0 && static_cast<size_t>(idIndex) < mReadState->idList.size());

    heif_item_id id = mReadState->idList[idIndex];

    // get image handle
    heif_image_handle* handlePtr = nullptr;
    auto error = heif_context_get_image_handle(mReadState->context.get(), id, &handlePtr);
...
int QHeifHandler::currentImageNumber() const
{
    ...
    return mReadState->currentIndex;
}
...
int QHeifHandler::imageCount() const
{
    ...
    return static_cast<int>(mReadState->idList.size());
}
...
bool QHeifHandler::jumpToImage(int index)
{
    ...
    if (index < 0 || static_cast<size_t>(index) >= mReadState->idList.size())
        return false;

    mReadState->currentIndex = index;
    return true;
}
...
bool QHeifHandler::jumpToNextImage()
{
    ...
    return jumpToImage(mReadState->currentIndex + 1);
}
...
bool QHeifHandler::write(const QImage& preConvSrcImage)
{
    ...
    {
        const uint8_t* srcBegin = srcData + y * srcStride;
        const uint8_t* srcEnd = srcBegin + srcStride;
    ...
    error = heif_encoder_set_lossy_quality(encoder.get(), mQuality);
    if (error.code)
    {
